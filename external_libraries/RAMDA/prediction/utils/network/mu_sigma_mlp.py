...
class MUSIGMA_MLP(nn.Module):
    ...
    def predict_proba(self, mu_sigma):
        y_score = self.mu_sigma_linear(mu_sigma)
        y_proba = torch.softmax(y_score, dim=1)
        return y_proba


def musigma_mlp_train(
    model_path,
    ...
    **kwargs,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in range(total_batch):
            ...
            batch_x_data = batch_input[:, :]
            batch_x_label = np.concatenate(
            ...
            batch_x_data, batch_x_label = (
                batch_x_data.to(device),
                batch_x_label.to(device),
            )
        ...
        print(
            ...
            % (epoch, mean_loss, time_end - time_start)
        )
        # Save the model
        ...
        y_scores = musigma_mlp_evaluate(
            test_data, model, vae_model, **kwargs
        )
        ...
        # print(
        #     "[MLP] Testing:\t[accuracy, recall, precision, f1, time]=[{:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.2f}s]".format(
        #         ret_test["accuracy"],
        #         ret_test["recall"],
        #         ret_test["precision"],
        #         ret_test["f1"],
        ...
        #     )
        # )
        # print(classification_report(y_labels, y_pred, digits=4))
    return y_scores


def musigma_mlp_evaluate(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        for batch_idx in range(total_batch):
            ...
            y_probs = model.predict_proba(mu_sigma)
            # y_pred.append((y_probs[:, 1] > threshold).long())
            y_scores.append(y_probs[:, 1])
