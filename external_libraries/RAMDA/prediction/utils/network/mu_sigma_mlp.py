...
def musigma_mlp_train(model_path, train_data: RAMDADataset, test_data: RAMDADataset, model: MUSIGMA_MLP, vae_model: FD_VAE, logger: Logger, **kwargs):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx in range(total_batch):
            ...
            batch_x_label = np.concatenate((np.zeros(len(batch_benign_data)), np.ones(len(batch_malware_data))), axis=0)
        ...
        print('Epoch: %d, Loss: %.4f, Time: %.2f' % (epoch, mean_loss, time_end - time_start))
        # Save the model
        ...
        print("[MLP] Testing:\t[accuracy, recall, precision, f1, time]=[{:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.2f}s]".format(ret_test['accuracy'], ret_test['recall'], ret_test['precision'], ret_test['f1'], test_end_time - test_start_time))
